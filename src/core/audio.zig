...
//! mirroring `TraceSink`: opaque ctx + fn pointer + thin `write` wrapper, no
//! allocation, no state, no work when null. The seam (not the bool-per-frame
//! payload) is what carries forward to Game Boy's APU sink — see ADR 0015
//! and the corresponding bullet in `docs/next-target-handoff.md`.
